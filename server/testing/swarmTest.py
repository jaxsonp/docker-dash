...
if __name__ == "__main__":
  ...
  print()
  test("Get app names - success",                        "GET", f"{BASE_URL}/{FACILITY_ID}/get-app-names", 200)
  test("Get app names - invalid facility ID",            "GET", f"{BASE_URL}/iaminvalid/get-app-names", 400)

  print()
  test("Get service status - success",                   "GET", f"{BASE_URL}/{FACILITY_ID}/get-app-status", 200)
  test("Get service status - success (specific app)",    "GET", f"{BASE_URL}/{FACILITY_ID}/get-app-status?name={APP_NAME}", 200)
  test("Get service status - invalid facility ID",       "GET", f"{BASE_URL}/iaminvalid/get-app-status?name={APP_NAME}", 400)
  test("Get service status - invalid app name",          "GET", f"{BASE_URL}/{FACILITY_ID}/get-app-status?name=iaminvalid", 400)

  print()
  test("Get service stats - success",                    "GET", f"{BASE_URL}/{FACILITY_ID}/get-app-stats", 200)
  test("Get service stats - success (specific app)",     "GET", f"{BASE_URL}/{FACILITY_ID}/get-app-stats?name={APP_NAME}", 200)
  test("Get service stats - invalid facility ID",        "GET", f"{BASE_URL}/iaminvalid/get-app-stats?name={APP_NAME}", 400)
  test("Get service stats - invalid app name",           "GET", f"{BASE_URL}/{FACILITY_ID}/get-app-stats?name=iaminvalid", 400)
  deleteApp(f"{IMAGE_NAME}--{USER_NAME}2")
  deleteApp(f"{IMAGE_NAME}--{USER_NAME}3")

  print()
  test("Get service info - success",                     "GET", f"{BASE_URL}/{FACILITY_ID}/get-app-info?name={APP_NAME}", 200)
  test("Get service info - invalid facility ID",         "GET", f"{BASE_URL}/iaminvalid/get-app-info?name={APP_NAME}", 400)
  test("Get service info - invalid app name",            "GET", f"{BASE_URL}/{FACILITY_ID}/get-app-info?name=iaminvalid", 400)
  test("Get service info - no app name",                 "GET", f"{BASE_URL}/{FACILITY_ID}/get-app-info", 400)

  print()
  test("Kill service - invalid facility ID",             "POST", f"{BASE_URL}/iaminvalid/kill-app?name={APP_NAME}", 400)
  test("Kill service - invalid app name",                "POST", f"{BASE_URL}/{FACILITY_ID}/kill-app?name=iaminvalid", 400)
  test("Kill service - no app name",                     "POST", f"{BASE_URL}/{FACILITY_ID}/kill-app", 400)
  test("Kill service - success",                         "POST", f"{BASE_URL}/{FACILITY_ID}/kill-app?name={APP_NAME}", 200)

  print()
  test("Get node names - success",                       "GET", f"{BASE_URL}/{FACILITY_ID}/get-node-names", 200)
  test("Get node names - invalid facility ID",           "GET", f"{BASE_URL}/iaminvalid/get-node-names", 400)

  print()
  test("Get node status - success",                      "GET", f"{BASE_URL}/{FACILITY_ID}/get-node-status", 200)
  test("Get node status - success (specific node)",      "GET", f"{BASE_URL}/{FACILITY_ID}/get-node-status?hostname={NODE_NAME}", 200)
  test("Get node status - invalid facility ID",          "GET", f"{BASE_URL}/iaminvalid/get-node-status?hostname={NODE_NAME}", 400)
  test("Get node status - invalid app name",             "GET", f"{BASE_URL}/{FACILITY_ID}/get-node-status?hostname=iaminvalid", 400)

  print()
  test("Get node info - success",                        "GET", f"{BASE_URL}/{FACILITY_ID}/get-node-info?hostname={NODE_NAME}", 200)
  test("Get node info - invalid facility ID",            "GET", f"{BASE_URL}/iaminvalid/get-node-info?hostname={NODE_NAME}", 400)
  test("Get node info - invalid app name",               "GET", f"{BASE_URL}/{FACILITY_ID}/get-node-info?hostname=iaminvalid", 400)
  test("Get node info - no app name",                    "GET", f"{BASE_URL}/{FACILITY_ID}/get-node-info", 400)
