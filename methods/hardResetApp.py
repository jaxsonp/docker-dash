...
def hardResetApp(facility_id, app_name="", app_id="") -> flask.Response:
  ...
  completedProcess = subprocess.run(f"docker inspect {app_name} --format \"{{{{.Config.Image}}}}\"", shell=True, capture_output=True)
  ...
  completedProcess = subprocess.run(f"docker create --name \"{app_name}\" --pull never \"{image_name}\"", shell=True, capture_output=True)
  if completedProcess.returncode != 0:
