...
def createApp(facility_id) -> flask.Response:
  ...
  completedProcess = subprocess.run(f"docker image ls --format \"{{{{.Repository}}}}\"", shell=True, capture_output=True)
  ...
  completedProcess = subprocess.run(f"docker create --name \"{container_name}\" --pull never {image_name}", shell=True, capture_output=True)
