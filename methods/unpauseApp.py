...
def unpauseApp(facility_id, app_name="", app_id="") -> flask.Response:
  ...
  completedProcess = subprocess.run(f"docker ps -a -f id={app_id} --format \"{{{{.State}}}}\"", shell=True, capture_output=True)
  if completedProcess.stdout.decode() != "paused\n":
    return flask.make_response("App must be paused to be unpaused", 409)

  # executing system command
  completedProcess = subprocess.run(f"docker unpause {app_id}", shell=True, capture_output=True)
