...
def getAppStatus(facility_id, app_name="", app_id="") -> flask.Response:
  ...
  if app_name == None:
    ...
    completedProcess = subprocess.run(f"docker ps -a --format json", shell=True, capture_output=True)
    ...
    # executing system command
    completedProcess = subprocess.run(f"docker ps -a -f id={app_id} --format json", shell=True, capture_output=True)
    if completedProcess.returncode != 0:
      return flask.make_response(f"Failed to query app:\n"+completedProcess.stdout.decode()+"\n"+completedProcess.stderr.decode(), 500)
